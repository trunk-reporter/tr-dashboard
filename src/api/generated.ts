...
export interface paths {
    ...
    "/admin/maintenance/config/{key}": {
         ...
         *     coded default. Returns 409 if the key is locked by an environment
         *     variable.
         *     Requires the `admin` scope.
         */
    ...
    "/admin/audit-log": {
         ...
         *     bytes with a "…(truncated, N bytes)" marker. (Keep the request
         *     line and headers under 64 KiB: the server's limit is 64 KiB plus
         *     Go's 4 KiB read slack, and a larger request gets 431.)
         *
...
export interface components {
    schemas: {
        ...
        /** @description Active retention settings for automatic maintenance. Each setting includes a `_source` ("env", "db", or "default") and a `_locked` flag (true when set by environment variable and immutable through the API). `_source` follows `PUT /admin/maintenance/config` and `DELETE /admin/maintenance/config/{key}` at once, without a restart - "db" right after a PUT, "default" right after a DELETE. A setting from an environment variable ("env") is locked, and PUT and DELETE return 409 for it. */
        MaintenanceConfig: {
            /**
